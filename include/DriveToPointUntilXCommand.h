...
enum DriveToPointXExitDirection {
    DRIVE_TO_POINT_EXIT_LEFT_OF_X,
    DRIVE_TO_POINT_EXIT_RIGHT_OF_X
};

struct DriveToPointUntilXTarget {
    float target_x;
    float target_y;
    float exit_x;
    DriveToPointXExitDirection exit_direction;
};

class DriveToPointUntilXCommand : public Command {
private:
    Drivetrain& drivetrain;
    PositionTracking& positionTracking;
    DriveToPointUntilXTarget target;
    ...
    bool crossedExitLine() {
        if (target.exit_direction == DRIVE_TO_POINT_EXIT_LEFT_OF_X) {
            return currentX <= target.exit_x;
        }
...
public:
    ...
    void initialize() override {
        setCommandStatus("Drive To Point Until X");
        currentState = DRIVE_TO_POINT_POINTING;
